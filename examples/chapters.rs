...
fn main() {
    ...
    match ffmpeg_rs::format::input(&env::args().nth(1).expect("missing input file name")) {
        Ok(ictx) => {
            ...
            let mut octx = ffmpeg_rs::format::output(&"test.mkv").expect("Couldn't open test file");
